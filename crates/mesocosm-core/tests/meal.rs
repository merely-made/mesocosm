...
use mesocosm_core::{Intent, Outcome, Rejection, Route, World, snapshot, state_hash};
...
fn incorporating_gives_body_and_no_energy() {
    ...
    let outcome = world.apply(Intent::Metabolize { organism: prey, route: Route::Incorporate });
...
fn the_same_meal_cannot_be_spent_twice() {
    ...
    for route in [Route::Burn, Route::Incorporate] {
        let (mut world, prey) = fed();
...
fn burning_and_growing_diverge_from_one_world() {
    ...
    grown.apply(Intent::Metabolize { organism: prey, route: Route::Incorporate });
...
fn venom_is_charged_whatever_the_meal_becomes() {
    ...
    let burned_energy = {
        ...
        // Burning nets the mass and still pays the venom.
        assert_eq!(world.energy_mg, before.saturating_sub(venom) + mass);
        world.energy_mg
    };
...
fn a_refused_meal_leaves_the_world_untouched() {
    ...
    let outcome = world.apply(Intent::Metabolize { organism: absent, route: Route::Incorporate });
